...
- Active window:
  - `2026-02-16` to `2026-04-08`
- This snapshot preserves the pre-rollover live engineering timeline so the detailed task-by-task record remains available while the new `DEVLOG.md` stays short and focused.
- Key work completed across this snapshot:
  - Gradient-05 frontend and visualization work around control panels, EE/pose rendering, diagnostics placement, and trajectory authoring UX
  - EtherCAT / RTCore bring-up, startup sequencing, power-up/down behavior, fault visibility, DS402 reset flows, and telemetry/debug instrumentation
  - Controller/API/runtime changes including SIM/LIVE hot switching, motion-path hardening, bounded setpoint behavior, and command-path cleanup
  - A6-EC native-home implementation, live debugging, RTCore offset/frame fixes, and subsequent commissioning regression investigations
  - Documentation and SOP updates that captured validated operating principles and commissioning safety guidance
...
## 2026-04-07 01:06 +0000

- Task summary:
  - Performed the post-reboot live validation, deployed the rebuilt RTCore binary into the systemd service path, and verified the full non-RT startup-drive-config readback flow on hardware.
- Changes:
  - Read repo memory and checked the live service state after reboot.
  - Confirmed `gradient-rt-motion.service` was active and that EtherCAT now converges cleanly to full OP with `responding=6/6`, `online=6/6`, `operational=6/6`, and `wkc=12/12`.
  - Found a deployment gap:
    - the rebuilt binary in the repo was `/home/pi/GradientOS/src/gradient_rt_motion/gradient-rt-motion`
    - the systemd service runs `/usr/local/bin/gradient-rt-motion`
    - the installed service binary timestamp lagged the rebuilt repo binary
  - Installed the rebuilt RTCore binary into the service path:
    - `sudo install -m 0755 /home/pi/GradientOS/src/gradient_rt_motion/gradient-rt-motion /usr/local/bin/gradient-rt-motion`
  - Restarted the service and validated the new binary via journal strings:
    - saw `EtherCAT startup readback deferred ... metrics thread will verify after startup_ready`
    - saw `EtherCAT startup readback begin ... delay_ms=500`
    - saw per-axis readback success for all 6 axes with `commanded=1 readback=1 verified=1`
  - Verified `/run/gradient-rt-motion/metrics.json` reflects:
    - `startup_ready=1`
    - `operational_slaves=6`
    - `startup_drive_config.readback_valid=1`
    - `startup_drive_config.readback=1`
    - `startup_drive_config.verified=1`
  - Verified the higher-level launcher path:
    - `./start-stack.sh status` showed controller/api/web down before launch
    - launched `./start-stack.sh --headless`
    - launcher confirmed bus-ready, controller ready, API health ready, and runtime config sane
    - `./start-stack.sh probe` reported `physical_state: BUS_UP_DISARMED` with RTCore `UP` and EtherCAT master `OP`
- Validation:
  - `systemctl status gradient-rt-motion.service --no-pager -l`
  - `journalctl -u gradient-rt-motion.service --since '2026-04-07 01:05:25' --no-pager`
...
- Follow-up notes / risks:
  - The full launcher is currently running in `--headless` mode, so controller + API are up but the web UI was intentionally not started in this validation pass.
  - `start-stack.sh status` can show `launcher_state: absent` even when RTCore alone is already healthy, because RTCore systemd and the higher-level launcher are distinct layers.

## 2026-04-07 01:12 +0000

- Task summary:
  - Investigated the reported front-panel `ALF1.0` indication on J1 after the stack was already healthy.
- Changes:
  - Confirmed manual mapping:
    - `docs/resources/a6ec_manual_codes.md` defines `ALF1.0` as `Re-power-on required for parameter settings to take effect`
  - Checked live API and RTCore state while the user observed the display:
    - `metrics.json` still showed `startup_ready=1`, `responding=6/6`, `online=6/6`, `operational=6/6`, `wkc=12/12`
    - all axes still showed `startup_drive_config.commanded=1`, `readback=1`, `verified=1`
    - no active `manufacturer_error_code` was present in RTCore metrics / API joint detail
  - Interpretation:
    - this points to a drive-local pending-parameter-power-cycle reminder on J1 rather than a bus-wide EtherCAT bring-up failure or a failed startup-drive-config readback.
- Validation:
  - `rg "ALF1\\.0|Re-power-on required for parameter settings to take effect" docs/resources`
  - `ReadFile` on `/run/gradient-rt-motion/metrics.json`
  - API checks via local HTTP on `/info/status` and `/info/joints-detailed`
- Follow-up notes / risks:
  - If `ALF1.0` clears after a clean safe repower and does not return, that is consistent with a one-time parameter-apply reminder.
  - If `ALF1.0` returns on every startup, the next design improvement should be to avoid unconditional startup writes for settings that are already at the commanded value, or move such writes into an explicit commissioning flow.

## 2026-04-07 01:34 +0000

- Task summary:
  - Implemented a programmatic encoder-data reset path for the A6-EC profile, validated it with builds/tests, deployed it, and restored the live stack after a drive power cycle.
- Changes:
  - Mapped the vendor reset parameter from the local ESI:
    - `F31.10 Encoder data reset` is exposed at EtherCAT object `0x2031:0x11`
    - the manual-directed recovery value for the battery-fault case is `4`
  - Added drive-profile metadata:
    - `src/gradient_os/arm_controller/profiles/drive/a6ec_ds402.py`
      - added `ENCODER_DATA_RESET_OPERATION`
      - added `get_encoder_data_reset_operation()`
    - `src/gradient_os/arm_controller/profiles/registry.py`
      - added `get_drive_encoder_data_reset_operation()`
  - Added a backend capability:
    - `src/gradient_os/arm_controller/actuator_interface.py`
      - added `reset_encoder_data()`
    - `src/gradient_os/arm_controller/backends/ethercat_rtcore/backend.py`
      - added `reset_encoder_data()`
      - added generic IPC sender `_send_cmd_service_sdo_write(...)`
      - looked up the live drive profile and used the profile-defined operation metadata rather than hardcoding the A6-EC object map in the generic backend path
  - Added a generic RTCore helper-thread SDO write primitive:
    - `src/gradient_rt_motion/ipc_v1.hpp`
      - added `CmdServiceSdoWriteV1`
      - added `MSG_CMD_SERVICE_SDO_WRITE`
      - added `SERVICE_SDO_VALUE_U16`
    - `src/gradient_rt_motion/main.cpp`
      - added helper-thread `service_sdo_write_axes(...)`
      - added command-ring handling for `MSG_CMD_SERVICE_SDO_WRITE`
      - kept the SDO write out of `rt-cycle`
  - Added controller/API wiring:
    - `src/gradient_os/arm_controller/command_api.py`
      - added `handle_reset_encoder_data(...)`
      - mirrors the existing reset-fault safety sequence: STOP -> WAIT_FOR_IDLE -> backend action -> remain disarmed
    - `src/gradient_os/run_controller.py`
      - added `RESET_ENCODER_DATA` UDP/controller command handling
    - `src/gradient_os/api/main.py`
      - added `POST /control/reset-encoder-data`
  - Added targeted tests:
    - `tests/test_api_endpoints.py`
      - added route coverage for `/control/reset-encoder-data`
    - `tests/test_command_api_direct_setpoint.py`
      - added controller safety-wrapper coverage for `handle_reset_encoder_data`
  - Validation/deploy:
    - `make -C src/gradient_rt_motion`
      - passed
    - `./.venv/bin/python -m pytest tests/test_api_endpoints.py tests/test_command_api_direct_setpoint.py -q`
      - `96 passed`
    - `ReadLints` on all touched files
      - no diagnostics
    - installed rebuilt RTCore into `/usr/local/bin/gradient-rt-motion`
    - restarted `gradient-rt-motion.service`
    - confirmed `/control/reset-encoder-data` appears in `http://127.0.0.1:4000/openapi.json`
  - Live recovery note:
    - the first stack restart failed because the drives had been powered off, leaving RTCore at `responding=0/6 link_up=0`
    - after the user powered the drives back on and enough time elapsed for rediscovery, RTCore returned to `startup_ready=1` and full `6/6 OP`
    - relaunched `./start-stack.sh --headless` successfully on the updated code
- Validation:
  - `systemctl status gradient-rt-motion.service --no-pager -l`
  ...
  - `./start-stack.sh probe`
  - `./start-stack.sh --headless`
  - `python3` fetch of `http://127.0.0.1:4000/openapi.json`
- Follow-up notes / risks:
  - The encoder-data reset path is implemented and deployed, but the destructive drive-side reset itself has not been triggered yet in this session.
  - After using `/control/reset-encoder-data`, the intended operator flow is still: keep disarmed -> safe repower -> native re-home -> only then trust multi-turn absolute position again.

## 2026-04-07 01:40 +0000

- Task summary:
  - Executed the all-axis encoder-data reset, verified the drive-side SDO writes, and confirmed the stack recovered cleanly after the subsequent drive power cycle.
- Changes:
  - Confirmed preconditions before reset:
    - `BUS_UP_DISARMED`
    - `6/6 OP`
    - `startup_ready=1`
  - Triggered:
    - `POST /control/reset-encoder-data`
  - Verified RTCore helper-thread writes for all 6 axes:
    - `index=0x2031`
    - `subindex=0x11`
    - `value=4`
  - Observed the expected post-power-cycle EtherCAT rediscovery window after the user cycled the drives.
  - Confirmed recovery to:
    - `responding=6/6`
    - `online=6/6`
    - `operational=6/6`
    - `wkc=12/12`
    - `startup_ready=1`
  - Confirmed startup-drive-config readback re-verified successfully on all axes after recovery.
- Validation:
  - API request to `/control/reset-encoder-data`
  - `journalctl -u gradient-rt-motion.service -n 120 --no-pager`
  - `./start-stack.sh probe`
  - `ReadFile` on `/run/gradient-rt-motion/metrics.json`
- Follow-up notes / risks:
  - After encoder-data reset, the absolute multi-turn position should still be treated as untrusted until the joints are deliberately re-homed.
  - If the user has not physically placed the robot at a known home/reference pose, do not use native-home yet; first move each joint to a known mechanical reference.

## 2026-04-07 01:43 +0000

- Task summary:
  - Executed the native-home path on all axes after the encoder reset recovery and verified drive-side success on hardware.
- Changes:
  - Confirmed preconditions:
    - `BUS_UP_DISARMED`
    - `6/6 OP`
    - controller + API healthy
  - Issued native-home requests for joints 1 through 6 through `POST /control/home-joint-native`
  - Verified RTCore helper-thread success logs for each axis:
    - `EtherCAT native_home axis=<n> ... saved=1`
  - Verified metrics after the sequence:
    - `native_home_state=2` for all six axes
    - `native_home_last_abort_code=0` for all six axes
    - stack remained `DISARMED` and `6/6 OP`
- Validation:
  - API POSTs to `/control/home-joint-native` for joints `1..6`
  - `journalctl -u gradient-rt-motion.service -n 160 --no-pager`
  - `ReadFile` on `/run/gradient-rt-motion/metrics.json`
  - `./start-stack.sh probe`
- Follow-up notes / risks:
  - The user explicitly treated the current pose as “good enough home for now”, so this establishes a usable test home, not necessarily the final precision mechanical home.
  - If the robot is later moved to a better reference pose, rerun native-home per joint to refine the drive-side home reference.

## 2026-04-07 01:48 +0000

- Task summary:
  - Completed the encoder-retention before/after power-cycle verification after encoder reset and native-home.
- Changes:
  - Captured the before snapshot:
    - experiment id: `retention-20260407-014559`
    - path: `logs/encoder-retention/retention-20260407-014559/before_power_down.json`
  - User power-cycled the drives and reported the expected display progression:
    - `18rd` -> `28rd` -> `88rd`
  - Verified post-cycle stack recovery:
    - `6/6 OP`
    - `wkc=12/12`
    - `startup_ready=1`
    - startup drive-config readback still verified on all axes
  - Captured the after snapshot:
    - path: `logs/encoder-retention/retention-20260407-014559/after_power_up.json`
  - Generated comparison artifacts:
    - `logs/encoder-retention/retention-20260407-014559/comparison.json`
    - `logs/encoder-retention/retention-20260407-014559/comparison.md`
- Validation:
  - API POST to `/control/encoder-retention/capture` for `before_power_down`
  - API POST to `/control/encoder-retention/capture` for `after_power_up`
  - `./start-stack.sh probe`
  - `ReadFile` on `/run/gradient-rt-motion/metrics.json`
- Results:
  - `raw_encoder_mismatch = false`
  - `logical_angle_mismatch = false`
  - `startup_drive_config_mismatch = false`
  - all six axis raw counts matched exactly (`delta=0`)
  - all six logical joint angles matched exactly (`delta=0.0`)
  - `active_battery_or_multiturn_faults = []`
- Follow-up notes / risks:
  - This is a strong validation of the current absolute-encoder retention path for the present home pose and current startup configuration.
  - The stored home used here is a “good enough for now” test home; repeat the retention workflow after any future refinement of the mechanical reference/home pose.

## 2026-04-07 01:52 +0000

- Task summary:
  - Investigated a report that clicking the UI arm button appeared to do nothing.
- Changes:
  - Verified the manual stack was healthy:
    - RTCore `UP`
    - API `up`
    - controller `up`
    - EtherCAT `6/6 OP`
    - `BUS_UP_DISARMED`
  - Inspected the live launcher logs:
    - API log showed `POST /control/power-up` with HTTP 200
    - shortly after, API log also showed `POST /control/power-down` with HTTP 200
  - Inspected the controller log:
    - saw `Received SAFE_POWER_UP command`
    - saw `Backend safe power-up command sent`
    - later saw `Received SAFE_POWER_DOWN command`
    - saw the expected stop / wait-for-idle / backend safe power-down messages
  - Interpretation:
    - the UI did reach the backend; the final disarmed state was caused by a follow-on power-down/toggle rather than the arm request being ignored.
- Validation:
  - `./start-stack.sh probe`
  - `ReadFile` on `logs/startups/20260407-015133/api.log`
  - `ReadFile` on `logs/startups/20260407-015133/controller.log`
- Follow-up notes / risks:
  - If the user wants this reproduced again, watch the logs during a single click and avoid a second click so the final state is unambiguous.

## 2026-04-07 02:04 +0000

- Task summary:
  - Fixed drive-state telemetry so GradientOS reports actual drive feedback instead of treating requested enable flags as proof that the drives are energized.
- Changes:
  - Updated `src/gradient_os/telemetry/drive_faults.py`:
    - `physical_state` now becomes `ACTIVE` only when one or more axes are actually `OperationEnabled`
    - `driver_state` now reflects actual feedback-driven state (`FAULTED` / `ACTIVE` / `DISARMED` / `INACTIVE`) instead of `armed` / `axis_enable_mask`
    - added separate request/feedback counters: `enable_requested`, `requested_axes`, `statusword_feedback_axes`, `slave_online_axes`, and `slave_operational_axes`
  - Updated `web-ui/src/ControlPanel.tsx`, `web-ui/src/liveState.tsx`, and `web-ui/src/App.tsx`:
    - UI now treats drive power as active from actual feedback (`op_enabled_axes`) instead of the old request-flag shortcut
    - UI shows request-vs-actual detail in drive-status titles/details and blocks duplicate power-up requests while still allowing power-down to clear a stuck request
  - Added regression coverage:
    - `tests/test_drive_faults.py` verifies an enable request with zero DS402 feedback still reports `driver_state=DISARMED`
    - `web-ui/src/ControlPanel.test.tsx` verifies requested enable is kept separate from actual drive feedback in the UI
- Validation:
  - `./.venv/bin/python -m pytest tests/test_drive_faults.py tests/test_run_controller_helpers.py`
  - `npm test -- --run src/ControlPanel.test.tsx` (from `web-ui/`)
  - `ReadLints` on edited Python and UI files
  - `./start-stack.sh probe`
- Follow-up notes / risks:
  - The live Vite UI hot-reloaded the `ControlPanel` changes during editing, but any already-running Python API/controller process may need a restart to load the updated telemetry builder depending on how the stack was launched.
  - The current live probe still shows all six axes `ds402=NotReady` with `sw=0x0000` while EtherCAT is `OP`, so the next debugging step remains the RTCore/drive feedback path itself rather than the UI state presentation.

## 2026-04-07 02:11 +0000

- Task summary:
  - Fixed the actual RTCore arming failure by correcting the A6-EC `0x1B02` TxPDO layout; the prior layout falsely inserted `0x203F`, shifted every feedback offset, and caused RTCore to read zeros/wrong fields instead of the real statusword and position.
- Changes:
  - Updated `src/gradient_os/arm_controller/ethercat_drive_catalog.py`:
    - removed `manufacturer_err|0x203F|0x00|32` from the A6-EC cyclic `tx_pdo_layout`
    - left `manufacturer_error_code` as an optional/non-cyclic field until a proper separate acquisition path is added
  - Updated `tests/test_rtcore_runtime.py`:
    - corrected the expected rendered `GRADIENT_RT_DRIVE_TX_PDO_LAYOUT` string to start `err -> sw -> pos`
  - Synced the live RTCore deployment:
    - ran `./systemd/rt-motion/sync-runtime.sh --ensure-active` so `/etc/default/gradient-rt-motion` was regenerated and `gradient-rt-motion.service` restarted with the corrected TxPDO layout
- Validation:
  - `./.venv/bin/python -m pytest tests/test_rtcore_runtime.py tests/test_drive_faults.py tests/test_run_controller_helpers.py`
  - `ReadLints` on the edited files
  - live RTCore journal now shows corrected offsets for axis 0: `err=19 sw=21 pos=23 di=40` with `mfg_err=4294967295`
  - live `/run/gradient-rt-motion/metrics.json` now reports real cyclic feedback (`statusword=5712`, nonzero `pos_counts`) instead of all-zero feedback
  - live `POST http://127.0.0.1:4000/control/power-up` returned HTTP 200 / accepted
  - live `./start-stack.sh probe` after power-up reported:
    - `driver_state: ACTIVE`
    - `physical_state: ACTIVE`
    - `op_enabled_axes=6/6`
    - all six axes `ds402=OperationEnabled` with `sw=0x1637`
- Follow-up notes / risks:
  - This fix intentionally prioritizes correct cyclic feedback and working DS402 enable sequencing. A6-EC `0x203F` is not part of fixed TxPDO `0x1B02`, so precise manufacturer fault telemetry should be reintroduced through a separate non-RT SDO/service path rather than by re-breaking the PDO layout.
  - The current verification run was started `--headless` to minimize variables while proving the hardware arm path. If the local web UI should also be served from this machine right now, start the non-headless launcher or separately launch the web tier.

## 2026-04-07 02:14 +0000

- Task summary:
  - Promoted the validated A6-EC PDO-layout / byte-shift lesson into the canonical GradientOS skill so future RTCore/EtherCAT work keeps that failure mode in view.
- Changes:
  - Updated `.cursor/skills/gradientos-sop/rtcore-ethercat.md`:
    - added explicit PDO-layout guardrails
    - documented that A6-EC fixed TxPDO `0x1B02` does not include `0x203F`
    - documented the failure signature where EtherCAT stays healthy but cyclic feedback becomes implausible due to shifted offsets
  - Updated `.cursor/skills/gradientos-sop/RTOS_ETHERCAT_MASTER_OPERATING_PRINCIPLES.md`:
    - clarified that the drive catalog PDO map must exactly match the assigned drive PDO
    - clarified that "carry both generic and manufacturer faults" does not justify forcing `0x203F` into cyclic `0x1B02`
    - added a debugging lesson describing the 4-byte-shift failure mode and the need to validate plausible cyclic values, not just bus `OP`
- Validation:
  - manual review of the updated skill files after the previously validated live hardware fix
  - no code-path changes in this step; documentation-only consolidation
- Follow-up notes / risks:
  - Manufacturer-specific A6-EC fault detail remains an intentional follow-up for a separate non-RT acquisition path if we want precise `0x203F` reporting without disturbing cyclic PDO truth.

## 2026-04-07 02:20 +0000

- Task summary:
  - Investigated the user's report of a slight arm movement on power-up after the RTCore/EtherCAT fix.
- Findings:
  - `src/gradient_os/arm_controller/backends/ethercat_rtcore/backend.py` still performs the same high-level RTCore power-up sequence:
    - synchronize command targets to feedback
    - `arm=true`
    - set CSP mode
    - enable axes
  - `src/gradient_rt_motion/main.cpp` still has no implemented brake state machine; status snapshots default `brake_state` to `BRAKE_UNKNOWN`
  - repo docs/plan still place brake sequencing primarily on the A6-EC drive-side BK output path, with relevant drive timing parameters:
    - `C05.13` brake-release-to-command delay
    - `C05.10` brake-apply to motor de-energized delay
- Interpretation:
  - no evidence was found of a newly introduced software sequence that explicitly powers/releases brakes before motor energization
  - the more likely explanation is that the earlier broken PDO layout prevented true enable behavior, while the fixed system now reaches genuine `OperationEnabled`, exposing the real physical brake/torque handoff behavior
- Validation:
  - code inspection of:
    - `src/gradient_os/arm_controller/backends/ethercat_rtcore/backend.py`
    - `src/gradient_rt_motion/main.cpp`
    - `src/gradient_rt_motion/ds402.hpp`
    - `docs/ethercat/bringup.md`
    - `RTOS-ETHERCAT-PLAN/RTOS-ETHERCAT-plan.md`
- Follow-up notes / risks:
  - a slight twitch on power-up now looks like a legitimate brake-handoff/timing topic worth addressing explicitly, rather than proof of a new request-path regression
  - if this becomes a fix task, next work should likely verify A6-EC BK configuration/delays and consider adding explicit brake-aware power-transition observability/logic instead of guessing from DS402 state alone

## 2026-04-07 18:27 +0000

- Task summary:
  - Hid the software `Zero` commissioning action from the default frontend UI while keeping the GradientOS software-zero path implemented and available behind an explicit Settings toggle.
- Changes:
  - Updated `web-ui/src/ControlPanel.tsx`:
    - added `showSoftwareZeroButton` as an explicit UI prop
    - hid the `Zero` button by default while keeping `Drive Home` visible
    - updated native-home success copy so it no longer assumes the `Zero` action is visible in the panel
  - Updated `web-ui/src/App.tsx`:
    - added persisted setting `showSoftwareZeroButton` with default `false`
    - exposed a General settings checkbox `Show software zero button in commissioning panel`
    - wired the persisted toggle into the live `ControlPanel`
  - Updated `web-ui/src/ControlPanel.test.tsx`:
    - added coverage proving software zero is hidden by default and appears only when the setting is enabled
  - Updated `docs/ethercat/bringup.md`:
    - documented that software zero remains supported but is hidden by default in the frontend
    - documented the Settings toggle used to re-expose it
  - Updated `.cursor/skills/gradientos-sop/RTOS_ETHERCAT_MASTER_OPERATING_PRINCIPLES.md`:
    - aligned commissioning/UI guidance with the new hidden-by-default software-zero policy
- Validation:
  - `cd /home/pi/GradientOS/web-ui && npm test -- --run src/ControlPanel.test.tsx`
  - `ReadLints` on:
    - `web-ui/src/App.tsx`
    - `web-ui/src/ControlPanel.tsx`
    - `web-ui/src/ControlPanel.test.tsx`
- Follow-up notes / risks:
  - This change intentionally keeps software zero in the controller/API for advanced workflows, so future UI work should preserve the distinction instead of silently removing the backend capability.

## 2026-04-07 18:36 +0000

- Task summary:
  - Zeroed the currently stored software joint offsets and fixed the RTCore/UI joint-feedback path so stale cached counts are no longer presented as live native-home truth.
- Changes:
  - Updated `.gradient_joint_zero_offsets.json`:
    - reset `gradient-05.logical_joint_master_offsets_rad` to all zeros
  - Updated `src/gradient_os/arm_controller/backends/ethercat_rtcore/backend.py`:
    - added fresh-feedback gating for RTCore joint reads using recent status snapshots, positive WKC, and nonzero statusword evidence
    - made `get_joint_positions()` reject unavailable/stale RTCore feedback instead of silently returning cached axis counts as live joint truth
    - made `sync_read_positions()` return no raw positions when RTCore feedback is stale
  - Updated `src/gradient_os/run_controller.py`:
    - kept monitor telemetry publishing even when live joint feedback is unavailable
    - omitted `joints` and published a joint-feedback availability/error marker instead of silently reusing dead joint data
  - Updated `web-ui/src/ControlPanel.tsx`:
    - switched fallback joint polling to `/info/joints-detailed`
    - rejected `read_source != live_feedback`
    - cleared commissioning joint feedback and surfaced a cached-only warning instead of treating cached controller state as live
  - Updated `web-ui/src/App.tsx`:
    - allowed fallback joint sync to explicitly clear `latest.joints`
    - added a stage overlay warning when live joint feedback is unavailable and the displayed pose may be stale
  - Updated tests:
    - `web-ui/src/ControlPanel.test.tsx` now mocks `/info/joints-detailed`
    - `tests/test_gradient05_limits_and_backends.py` includes a stale RTCore feedback regression test
- Validation:
  - `cd /home/pi/GradientOS/web-ui && npm test -- --run src/ControlPanel.test.tsx`
  - `./.venv/bin/python -m pytest tests/test_gradient05_limits_and_backends.py -k stale_joint_feedback`
  - `./.venv/bin/python -m pytest tests/test_gradient05_limits_and_backends.py -k zero_capture_persists_joint_offsets`
  - `ReadLints` on edited Python/TypeScript files
  - attempted full `./.venv/bin/python -m pytest tests/test_gradient05_limits_and_backends.py`, but two unrelated pre-existing expectation mismatches failed:
    - `test_gradient05_config_defaults_and_mapping_shape`
    - `test_ethercat_backend_prefers_robot_defined_axis_scaling`
- Follow-up notes / risks:
  - The zero-offset file change will only affect already-running backend/controller processes after they reload or restart.
  - The 3D stage now warns when live joints are unavailable, but it intentionally does not invent a fake pose; it preserves the last rendered geometry and marks it as potentially stale.

## 2026-04-07 18:48 +0000

- Task summary:
  - Investigated why `Drive Home` on J2 dropped the brake/axis out and appeared to do nothing in the UI, then patched both the operator feedback gap and the RTCore post-home restore behavior in source.
- Findings:
  - `src/gradient_rt_motion/main.cpp` `MSG_CMD_NATIVE_HOME` handler explicitly cleared the requested axis from `axis_enable_mask` and could leave it disarmed after the home write.
  - Live `/run/gradient-rt-motion/metrics.json` showed J2 native-home had actually succeeded:
    - `native_home_state=2`
    - nonzero `native_home_position_offset`
    - requested axis removed from the active enable mask
  - The frontend had no native-home telemetry path, so the operator saw the brake/enable side effect but not the success/failure result.
- Changes:
  - Updated `src/gradient_rt_motion/main.cpp`:
    - after a successful native-home operation, restore the previous `axis_enable_mask` and `armed` state instead of leaving the homed axis dropped out
    - log restore vs failure-to-restore explicitly
  - Updated `src/gradient_os/telemetry/drive_faults.py`:
    - carried `native_home_state`, `native_home_state_name`, `native_home_position_offset`, and native-home abort codes through the existing drive snapshot axis payload
  - Updated `web-ui/src/liveState.tsx`, `web-ui/src/App.tsx`, and `web-ui/src/ControlPanel.tsx`:
    - added native-home telemetry fields to the frontend types
    - rendered per-joint commissioning status like `Drive Home succeeded | offset ... | axis currently disarmed`
  - Updated tests:
    - `tests/test_drive_faults.py` covers native-home telemetry propagation
    - `web-ui/src/ControlPanel.test.tsx` covers visible native-home status rendering
- Validation:
  - `./.venv/bin/python -m pytest tests/test_drive_faults.py`
  - `cd /home/pi/GradientOS/web-ui && npm test -- --run src/ControlPanel.test.tsx`
  - `ReadLints` on edited frontend/telemetry files
- Follow-up notes / risks:
  - The RTCore C++ native-home restore change is patched in source but was not rebuilt/redeployed to the live `gradient-rt-motion` service during this turn, so the current machine will not exhibit the restored post-home enable behavior until RTCore is rebuilt and restarted.

## 2026-04-07 18:55 +0000

- Task summary:
  - Rebuilt and redeployed `gradient-rt-motion`, then used the direct RTCore drive-fault reset pathway to clear the persistent J2 front-panel `Er87.1` / bus `0xFF00` fault while keeping the robot disarmed.
- Runtime actions:
  - Built `src/gradient_rt_motion/gradient-rt-motion` with `make -j4`
  - Installed/restarted RTCore via `./systemd/rt-motion/sync-runtime.sh --ensure-active`
  - Confirmed RTCore returned to healthy bus state:
    - `armed=0`
    - `axis_enable_mask=0`
    - `responding/online/operational=6/6`
    - `wkc=18/12`
  - Attempted full launcher startup with `./start-stack.sh`
  - Observed startup preflight correctly refuse controller launch because J2 remained faulted after the automatic reset attempt
  - Issued a targeted direct RTCore fault reset:
...
- Findings:
  - Before the targeted reset, `/run/gradient-rt-motion/metrics.json` showed:
    - J2 `statusword=5688`
    - J2 `error_code=65280 (0xFF00)`
    - all axes still disarmed
  - After the targeted reset, `/run/gradient-rt-motion/metrics.json` showed:
    - J2 `statusword=5712 (0x1650)`
    - J2 `error_code=0`
    - all six axes in `SwitchOnDisabled`
    - robot still `armed=0`, `axis_enable_mask=0`
  - `./start-stack.sh probe` normalized the final state as:
    - `driver_state: DISARMED`
    - `physical_state: BUS_UP_DISARMED`
    - EtherCAT master `OP`, `startup_ready=1`
- Validation:
  - `systemctl status gradient-rt-motion.service --no-pager`
  - `journalctl -u gradient-rt-motion.service -n 60 --since '2 minutes ago' --no-pager`
  - `cat-equivalent via ReadFile` on `/run/gradient-rt-motion/metrics.json`
  - `./start-stack.sh probe`
- Follow-up notes / risks:
  - The direct J2 reset cleared the latched fault without any enable request, which confirms the safe reset path works as intended on the live machine.
  - The stack is not fully relaunched yet; RTCore is healthy and disarmed, but controller/API/web remain down until the next explicit startup attempt.

## 2026-04-07 19:02 +0000

- Task summary:
  - Captured the failed end-to-end result after the user clicked `Drive Home` again in the live UI: the previously rebuilt/redeployed RTCore still does not solve the operator-facing native-home problem.
- Findings:
  - User reported the same behavior remains:
    - displayed offset/angle still wrong
    - visualization still not at the true home position
    - joint still deactivates / faults during or after `Drive Home`
  - Current live `/run/gradient-rt-motion/metrics.json` shows:
    - `armed=1`
    - `axis_enable_mask=63`
    - J2 `native_home_state=2`
    - J2 `native_home_position_offset=-368092`
    - J2 `error_code=65280 (0xFF00)`
    - J2 `statusword=5688 (FAULT)`
  - Current live `GET /info/joints-detailed` shows:
    - `read_source="live_feedback"`
    - J2 `arm_deg=3.3985931396484377`
    - J2 `axis_error_code=65280`
  - This means the new source/build does execute and report native-home success, but the system still ends up with a faulted J2 and the UI/API still present a post-home angle the user says is physically wrong.
- Interpretation:
  - The earlier RTCore change that restores `axis_enable_mask`/`armed` after native home is insufficient by itself.
  - The most likely remaining issue is that after the drive-side home offset changes, RTCore/controller/UI are not fully re-synchronizing the commanded/held reference and logical pose interpretation, allowing a bad CSP target step and/or stale reference model immediately after the home write.
- Validation:
  - `ReadFile /run/gradient-rt-motion/metrics.json`
  - `curl -sS http://127.0.0.1:4000/info/joints-detailed`
  - user live UI report after an actual hardware `Drive Home` click
- Follow-up notes / risks:
  - Do not hand this off as "fixed but flaky"; the current live hardware behavior is still broken in the exact workflow the user cares about.
  - Next debugging should start from the latest live contradiction (`native_home_state=2` + saved offset + J2 fault + wrong live/UI angle), not from the earlier partial success on reset/rebuild.

## 2026-04-07 19:09 +0000

- Task summary:
  - Changed the native-home workflow to behave like a safe commissioning transition instead of immediately restoring energized RTCore axes into a shifted encoder reference, and updated the commissioning UI to wait for post-home live feedback before claiming the pose is refreshed.
- Changes:
  - Updated `src/gradient_os/arm_controller/backends/ethercat_rtcore/backend.py`:
    - changed `native_home_joint()` to first neutralize motion, disable all RTCore axes, and disarm before sending `MSG_CMD_NATIVE_HOME`
    - changed the path to abort the request if that pre-home neutralization fails
    - updated the operator log message to state that drives remain disarmed until an explicit safe power-up
  - Updated `web-ui/src/ControlPanel.tsx`:
    - changed the native-home confirmation text to explain that the workflow temporarily disarms the drives and requires a later power-up
    - added a short retry loop after `/control/home-joint-native` so the panel waits for post-home live joint feedback instead of doing a single immediate refresh against an asynchronous ACK
    - changed the success/error commissioning messages to describe the new disarmed-after-home expectation
  - Updated `tests/test_gradient05_limits_and_backends.py`:
    - added a regression test proving native-home disarms before the RTCore request
    - added a regression test proving native-home aborts if the pre-neutralization step fails
- Validation:
  - `"/home/pi/GradientOS/.venv/bin/python" -m py_compile "/home/pi/GradientOS/src/gradient_os/arm_controller/backends/ethercat_rtcore/backend.py" "/home/pi/GradientOS/tests/test_gradient05_limits_and_backends.py"` (passed)
  - `"/home/pi/GradientOS/.venv/bin/python" -m pytest tests/test_gradient05_limits_and_backends.py -q -k "native_home or safe_power"` (passed, 4 tests)
  - `npm run build` in `web-ui` (passed)
  - `ReadLints` on edited backend/frontend/test files (no diagnostics)
- Follow-up notes / risks:
  - This session could not perform a real post-fix hardware retry because `/run/gradient-rt-motion/metrics.json` was absent and `127.0.0.1:4000` was not reachable from the workspace shell.
  - If live validation still shows a post-home fault even with this forced disarm path, the remaining fix likely belongs in RTCore native-home completion semantics rather than only in Python/UI sequencing.

## 2026-04-08 04:56 +0000

- Task summary:
  - Reworked A6-EC native-home handling so the saved drive offset, Python logical joint conversion, and RTCore hold-target frame all agree, then live-validated the J2 commissioning flow on hardware.
- Changes:
  - Updated `src/gradient_rt_motion/main.cpp`:
    - changed native-home offset capture from `current_offset - pos_counts` to `-pos_counts`
    - initialized `native_home_position_offset` from drive `0x60B0` on RTCore startup so persisted home offsets are visible immediately after restart
    - changed successful `MSG_CMD_NATIVE_HOME` completion to leave only the homed axis disabled instead of restoring it into enable
    - changed hold-target alignment to use the drive target frame (`pos_counts - native_home_position_offset`) so `safe_power_up()` no longer reintroduces `0xFF00` after home
  - Updated `src/gradient_os/arm_controller/backends/ethercat_rtcore/backend.py`:
    - removed the Python-side “disable all axes / disarm all drives” native-home regression
    - applied `native_home_position_offset` to logical joint feedback conversion and logical command conversion using RTCore metrics
    - updated software-zero capture to sample the native-home-adjusted logical pose
  - Updated `web-ui/src/ControlPanel.tsx`:
    - changed Drive Home copy to state that only the homed axis remains disabled after success
  - Updated `tests/test_gradient05_limits_and_backends.py`:
    - replaced the old “disable all axes before native home” expectation with the neutralize-then-request flow
    - added a regression test that native-home offsets are applied to logical read/write conversion
- Validation:
  - `"/home/pi/GradientOS/.venv/bin/python" -m py_compile "/home/pi/GradientOS/src/gradient_os/arm_controller/backends/ethercat_rtcore/backend.py" "/home/pi/GradientOS/tests/test_gradient05_limits_and_backends.py"` (passed)
  - `"/home/pi/GradientOS/.venv/bin/python" -m pytest tests/test_gradient05_limits_and_backends.py -q -k "native_home or safe_power or applies_native_home_offsets"` (passed, 5 tests)
  - `make -j4` in `src/gradient_rt_motion` (passed twice; once for the offset/frame fix and once for the hold-target compensation fix)
  - `npm run build` in `web-ui` (passed)
  - `ReadLints` on edited backend/RTCore/frontend/test files (no diagnostics)
  - Live hardware checks:
    - recovered the bad preexisting J2 latch by zeroing `0x60B0`, then pulsing DS402 reset directly with `sudo ethercat download -p1 -t uint16 0x6040 0x00 128`
    - verified from a disarmed baseline that `POST /control/home-joint-native` on J2 produced `native_home_position_offset=-125690`, `error_code=0`, `statusword=0x1650`, and `/info/joints-detailed` J2 `arm_deg=0.0`
    - verified after the RTCore hold-target fix that `POST /control/power-up` kept J2 fault-free with `axis_enable_mask=63`, `error_code=0`, and compensated `0x607A=253338`
    - verified homing J2 while powered left `armed=1`, `axis_enable_mask=61` (`0x3d`), `error_code=0`, `statusword=0x1650`, and `/info/joints-detailed` J2 `arm_deg≈0.00052`
- Follow-up notes / risks:
  - The old bad offset/fault combination can survive restarts strongly enough to block launcher startup preflight; direct DS402 reset may be required before the corrected software path can be validated.
  - J2 still drifted slightly between the first home and subsequent re-home under power (`~0.05 deg` before the second capture), so if that small motion matters the next investigation should focus on brake/torque handoff or mechanical settle rather than native-home propagation.

## 2026-04-08 05:00 +0000

- Task summary:
  - Documented the now-validated A6-EC native-home persistence model and added repo-local vendor-reference guidance to the canonical GradientOS SOP.
- Changes:
  - Updated `.cursor/skills/gradientos-sop/commissioning-safety.md`:
    - clarified that A6-EC native home persists `0x60B0` through `0x1010:01`
    - clarified that saved drive home is distinct from battery-backed multi-turn encoder retention
    - added the in-repo A6-EC manual and fault-code docs to the first-reference list
  - Updated `.cursor/skills/gradientos-sop/rtcore-ethercat.md`:
    - added a rule to consult repo-local A6-EC references before web lookup for vendor semantics
    - added the validated rule that RTCore safe-enable alignment after native home must use the offset-aware target frame
  - Updated `.cursor/memory/AGENT_SCRATCHPAD.md`:
    - recorded the user correction that the A6-EC manual/codebook already live in-repo and should be checked first
- Validation:
  - Documentation-only update based on the live-validated native-home behavior from the preceding entry and the current in-repo vendor reference files
- Follow-up notes / risks:
  - If we later support a non-battery-backed A6-EC encoder mode, the canonical SOP should call out that alternate retention behavior explicitly instead of assuming `C00.07=1`.

## 2026-04-08 05:25 +0000

- Task summary:
  - Investigated a live J2 commissioning jog incident where a requested `1 deg` step produced a much larger apparent move and left the UI motion/jog flow stranded.
- Changes:
  - Updated `src/gradient_os/arm_controller/backends/ethercat_rtcore/backend.py`:
    - hardened `wait_for_trajectory_complete()` so it accepts terminal RTCore idle/completed status for the latest submitted trajectory even when the exact `active_traj_id == traj_id` window is missed
    - added a final timeout fallback check to avoid surfacing false trajectory timeouts after RTCore has already gone terminal/idle
  - Updated `src/gradient_os/arm_controller/command_api.py`:
    - added bounded direct-setpoint logging with current logical angles, target logical angles, RPM limit, duration, and point count so future commissioning incidents are reconstructible directly from controller logs
  - Updated `.cursor/memory/AGENT_SCRATCHPAD.md`:
    - recorded the false-timeout RTCore bounded-setpoint failure mode and the new completion-detection rule
- Evidence gathered:
  - `logs/startups/latest/controller.log` showed `APPLY_JOINT_SETPOINT` sending a bounded RTCore trajectory and then the open-loop executor thread raising `TimeoutError: Timed out waiting for RTCore trajectory 1 to complete`
  - `logs/startups/latest/api.log` showed the sequence `power-up -> home-joint-native -> power-down -> power-up -> /control/joint-jog -> /control/power-down`
  - live `metrics.json` and `/info/joints-detailed` showed the stack fault-free and disarmed afterward, with J2 reporting about `-4.47 deg`
  - a direct helper check confirmed the backend's native-home-aware conversion would have turned the logged J2 target into an RTCore raw target of about `92595` counts, so the false timeout path was the confirmed software bug fixed here
- Validation:
  - `PYTHONPATH=/home/pi/GradientOS/src /home/pi/GradientOS/.venv/bin/python -m pytest tests/test_gradient05_limits_and_backends.py -q -k 'wait_for_trajectory_complete or short_trajectory_completion or stale_previous_completion'` (passed)
  - `PYTHONPATH=/home/pi/GradientOS/src /home/pi/GradientOS/.venv/bin/python -m pytest tests/test_api_endpoints.py -q -k 'control_joint_jog'` (passed)
  - `PYTHONPATH=/home/pi/GradientOS/src /home/pi/GradientOS/.venv/bin/python -m py_compile src/gradient_os/arm_controller/backends/ethercat_rtcore/backend.py src/gradient_os/arm_controller/command_api.py tests/test_gradient05_limits_and_backends.py` (passed)
  - `ReadLints` on edited files (no diagnostics)
- Follow-up notes / risks:
  - This patch fixes the confirmed false-timeout/UI-stranding bug. The final `-4.47 deg` disarmed pose may still include post-timeout sag after power-down; a fresh live retest with the new bounded-setpoint logging is needed to separate any remaining commanded-motion error from mechanical drop after disarm.

## 2026-04-08 05:35 +0000

- Task summary:
  - Isolated the new "1 deg jog moves ~5 deg" regression to this week's native-home command-frame change, not to IK and not to the older RTCore queue-only motion migration.
- Changes:
  - Updated `src/gradient_os/arm_controller/backends/ethercat_rtcore/backend.py`:
    - removed native-home offset subtraction from `_axis_q_from_joint_positions()` so outgoing RTCore logical position commands stay in the same offset-aware logical frame as RTCore hold targets
    - backed out the overly permissive trajectory-complete fallback that had started treating a bare idle RTCore snapshot as completed motion before the commit sequence was visible
  - Updated `tests/test_gradient05_limits_and_backends.py`:
    - changed the native-home conversion regression test so feedback remains offset-aware while outgoing command targets do not double-apply the offset
    - removed the timeout-fallback test that covered the now-reverted overly broad completion rule
  - Updated `.cursor/memory/AGENT_SCRATCHPAD.md`:
    - recorded the validated rule that native-home offset is read-path compensation only for RTCore logical position commands
- Evidence gathered:
  - live terminal logs showed `/control/joint-jog` sending `current_deg=[..., -4.418, ...]` and `target_deg=[..., -3.418, ...]`, while the robot physically overmoved by roughly `4.5-5 deg`
  - numeric inspection showed J2 `native_home_offset` was about `-3.506 deg`; the broken write path turned the target into `+0.088 deg` in RTCore axis space instead of leaving it at the intended `-3.418 deg`
  - blame/history confirmed the queue-only RTCore migration and bounded `max_motor_rpm` path landed on 2026-03-21, while the new regression came from this week's native-home write-path change
  - the terminal snippet also showed my previous timeout workaround was too permissive because it logged `RTCore trajectory execution finished: state=idle traj_id=0 elapsed=0.001s` immediately after acceptance; that fallback was reverted here
- Validation:
  - `PYTHONPATH=/home/pi/GradientOS/src /home/pi/GradientOS/.venv/bin/python -m pytest tests/test_gradient05_limits_and_backends.py -q -k 'native_home_offsets or wait_for_trajectory_complete or short_trajectory_completion or stale_previous_completion'` (passed)
  - `PYTHONPATH=/home/pi/GradientOS/src /home/pi/GradientOS/.venv/bin/python -m pytest tests/test_api_endpoints.py -q -k 'control_joint_jog'` (passed)
  - `ReadLints` on edited backend/test files (no diagnostics)
  - numeric sanity check after patch showed the same logged J2 jog target now stays at `axis_q_target_j2_deg=-3.417987...` instead of being shifted by the `~3.5 deg` native-home offset
  - recovered the stack to a clean `BUS_UP_DISARMED` state after clearing wedged launcher/api leftovers from restart attempts
- Follow-up notes / risks:
  - The commissioning jog path is still queue-based because of the older RTCore queue-only redesign; this fix removes the new offset double-application, but a later cleanup should still restore a simpler bounded joint-step primitive for commissioning instead of routing it through queued trajectories.

## 2026-04-08 16:15 +0000

- Task summary:
  - Restored explicit RTCore velocity payloads for single-point position moves and corrected local validation workflow to use the repo's existing `start.sh` environment instead of dependency-resolution tooling.
- Changes:
  - Updated `src/gradient_os/arm_controller/backends/ethercat_rtcore/backend.py`:
    - added explicit `qd` plus `_TRAJ_POINTF_HAS_VELOCITY` to `set_joint_positions()` one-point trajectory payloads
    - added explicit `qd` plus `_TRAJ_POINTF_HAS_VELOCITY` to `set_single_actuator_position()` one-point trajectory payloads
    - preserved `speed`/`accel` through `prepare_sync_write_commands()` and `sync_write()` so callers no longer lose the requested move speed before the RTCore payload is built
    - added helper logic to derive bounded single-point joint/axis velocities from the requested legacy `speed`, current feedback/setpoint state, robot gear ratios, and the configured RTCore RPM clamp
  - Updated `tests/test_gradient05_limits_and_backends.py`:
    - extended the setpoint regression test to assert one-point RTCore payloads now include `qd` and velocity flags
    - added a regression test that `sync_write()` preserves the requested speed/acceleration instead of hardcoding `0.0`
    - added a regression test that single-axis RTCore position moves include explicit velocity in the payload
  - Updated `.cursor/memory/AGENT_SCRATCHPAD.md`:
    - recorded the user correction that local validation on this machine should use `source ./start.sh` / repo `.venv`, not `uv`
- Validation:
  - `source "./start.sh" && python -m pytest tests/test_gradient05_limits_and_backends.py -q -k 'applies_master_offsets_to_setpoints or sync_write_preserves_requested_speed or single_actuator_setpoint_includes_velocity or execute_joint_trajectory_enqueues_velocity_points'` (passed: 3 tests, 33 deselected)
  - `source "./start.sh" && python -m py_compile src/gradient_os/arm_controller/backends/ethercat_rtcore/backend.py tests/test_gradient05_limits_and_backends.py` (passed)
  - `ReadLints` on edited files (no diagnostics)
- Follow-up notes / risks:
  - I have not done a live hardware retry in this entry, so this still needs an on-robot jog verification against the J2 commissioning path.
  - The single-point velocity mapping now reuses repo semantics instead of omitting `qd`, but if the legacy `speed` scale for RTCore commissioning should be different from the current `0..4095 -> fraction of configured RT max rpm` interpretation, that mapping should be tightened against the exact pre-regression behavior.
