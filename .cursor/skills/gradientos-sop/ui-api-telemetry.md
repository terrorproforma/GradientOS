...
- `shaft_frame_reference_source` (per-axis): string tag on every canonical-truth snapshot. Values: `"paired_sdo_snapshot"` (atomic `0x6064` latched by RTCore at the moment of the multi-turn SDO read — expected steady-state) or `"live_pdo"` (fallback when RTCore has not yet published a paired snapshot for this axis, e.g., first cycles after boot or absolute-feedback poll disabled). Surface this field in operator diagnostics so a stuck `live_pdo` reading signals a missing paired latch rather than a drive fault.
- `shaft_frame_tolerance_counts` / `shaft_frame_tolerance_base_counts` / `shaft_frame_tolerance_motion_widen_counts` / `shaft_frame_velocity_counts_per_s` (per-axis): show the effective tolerance, the static base, the velocity-proportional widening, and the Python-side velocity estimate used to compute it. Useful when investigating a gate trip during motion.
- `command_roundtrip_reference_source` / `command_roundtrip_velocity_counts_per_s` (per-axis): mirror the above for the command-frame roundtrip gate, which also derives its `reference_q` from the paired `0x6064` when available.
