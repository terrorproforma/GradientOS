...
- Live encoder-retention-family faults (`Er20.1 .. Er20.9`, `ErA0.1`, `ALF9.0`) surface as `encoder_retention_fault_present` and block trust regardless of anchor agreement, because multi-turn integrity is the exact precondition the anchor path depends on. `ErA0.1` ("Multi-turn overflow fault") is explicitly called out as a primary retention signal by vendor email 4 Q2(a).
