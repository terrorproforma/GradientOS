...
def test_tangent_roll_changes_weld_orientations(monkeypatch, tmp_path):
    ...
    assert roll0_payload["weld"]["options"]["spin_angle_deg"] == pytest.approx(0.0)
    assert roll45_payload["weld"]["options"]["spin_angle_deg"] == pytest.approx(45.0)
...
def test_tangent_roll_defaults_to_zero_when_missing(monkeypatch, tmp_path):
    ...
    assert payload["weld"]["options"]["spin_angle_deg"] == pytest.approx(0.0)


def test_normal_angle_changes_weld_orientations(monkeypatch, tmp_path):
    monkeypatch.setattr(command_api, "RECORDED_TRAJ_DIR", str(tmp_path / "recorded"))
    monkeypatch.setattr(utils, "TRAJECTORY_CACHE_DIR", str(tmp_path / "cache"))
    monkeypatch.setattr(
        utils,
    ...
    def _run_for_normal(normal_deg: float):
        captured: dict[str, list[np.ndarray] | None] = {"orientations": None}
        ...
        payload = command_api.plan_preview_trajectory_points(
            points=[[0.0, 0.0, 0.0], [0.04, 0.0, 0.0], [0.08, 0.02, 0.0]],
            preview_name=f"__tool_normal_{normal_deg:.0f}__",
            weld_metadata={
                "type": "fillet",
                "options": {
                    "post_action": "none",
    ...
    normal90_orientation, normal90_payload = _run_for_normal(90.0)
    normal0_orientation, normal0_payload = _run_for_normal(0.0)

    assert not np.allclose(normal90_orientation, normal0_orientation)
    assert normal90_payload["weld"]["options"]["spin_angle_deg"] == pytest.approx(90.0)
    assert normal0_payload["weld"]["options"]["spin_angle_deg"] == pytest.approx(0.0)


def test_spin_angle_defaults_to_zero_when_missing(monkeypatch, tmp_path):
    monkeypatch.setattr(command_api, "RECORDED_TRAJ_DIR", str(tmp_path / "recorded"))
    monkeypatch.setattr(utils, "TRAJECTORY_CACHE_DIR", str(tmp_path / "cache"))
    monkeypatch.setattr(
        utils,
    ...
    payload = command_api.plan_preview_trajectory_points(
        points=[[0.0, 0.0, 0.0], [0.04, 0.0, 0.0]],
        preview_name="__tool_normal_default__",
        weld_metadata={
            "type": "fillet",
            "options": {
                "post_action": "none",
